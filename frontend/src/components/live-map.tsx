...
type LiveMapProps = { snapshot: DriverSnapshot | null; sessionKey: string; stale?: boolean; drivers?: ActiveDriverSession[]; onSelect?: (id: string) => void; selectionDisabled?: boolean; showStatus?: boolean };
const MAX_TRAIL_POINTS = 250;
const FRESH_TELEMETRY_SECONDS = 5;
function driverColor(status: string | null | undefined, fresh: boolean) {
  if (!fresh) return '#d45508';
  if (status === 'DROWSY' || status === 'FATIGUED') return '#ce303d';
  if (status === 'ALERT') return '#17865b';
  return '#1746a2';
}

function MapControls({ position, fleetPositions = [] }: { position?: Position; fleetPositions?: Position[] }) {
  const map = useMap();
  ...
  useEffect(() => {
    ...
    } else if (!position && fleetPositions.length && !centered.current) {
      map.fitBounds(fleetPositions, { padding: [45, 45], maxZoom: 14 });
      centered.current = true;
    }
  }, [map, position, fleetPositions]);
...
function SessionMap({ snapshot, stale = false, drivers = [], onSelect, selectionDisabled = false, sessionKey, showStatus = true }: LiveMapProps) {
  ...
  const locationColor = driverColor(snapshot?.fatigue?.fatigue_status, fresh);
  const locatedDrivers = drivers.filter(driver => typeof driver.latitude === 'number' && Number.isFinite(driver.latitude) && Math.abs(driver.latitude) <= 90 && typeof driver.longitude === 'number' && Number.isFinite(driver.longitude) && Math.abs(driver.longitude) <= 180);
  ...
  return <div className="live-session-map relative isolate flex w-full flex-col overflow-hidden bg-[#f5f8fc]">
    <MapContainer center={position ?? [-2.5, 118]} zoom={position ? 15 : 5} scrollWheelZoom={false} className="z-0 min-h-[340px] w-full flex-1" style={{ height: '100%' }} aria-label="Peta posisi kendaraan dan jejak GPS sesi aktif">
      ...
      {position && <CircleMarker center={position} radius={10} eventHandlers={{ click: () => { if (!selectionDisabled && sessionKey) onSelect?.(sessionKey); } }} pathOptions={{ className: `driver-marker driver-marker-${sessionKey}`, color: '#fff', weight: 3, fillColor: locationColor, fillOpacity: 1 }}>
    ...
    {showStatus && <div className="absolute bottom-7 left-3 right-3 z-[500] flex flex-wrap items-center justify-between gap-2 rounded-lg border border-white/80 bg-white/95 px-3 py-2 text-[11px] shadow-sm backdrop-blur">
