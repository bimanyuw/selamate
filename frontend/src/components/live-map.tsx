...
type LiveMapProps = { snapshot: DriverSnapshot | null; sessionKey: string; stale?: boolean };
...
function SessionMap({ snapshot, stale = false }: Omit<LiveMapProps, 'sessionKey'>) {
  ...
  return <div className="relative isolate flex h-[360px] w-full flex-col overflow-hidden bg-[#f5f8fc]">
    <MapContainer center={position ?? [-2.5, 118]} zoom={position ? 15 : 5} scrollWheelZoom={false} className="z-0 min-h-[340px] w-full flex-1" style={{ height: '100%' }} aria-label="Peta posisi kendaraan dan jejak GPS sesi aktif">
      ...
      {position && <CircleMarker center={position} radius={8} pathOptions={{ color: '#fff', weight: 3, fillColor: locationColor, fillOpacity: 1 }}>
...
export default function LiveMap({ snapshot, sessionKey, stale = false }: LiveMapProps) {
  return <SessionMap key={sessionKey} snapshot={snapshot} stale={stale} />;
}
