...
function App() {
  ...
  return <div className="mx-auto max-w-6xl px-5 sm:px-8">
    ...
    <main className="pb-12">
      <section className="flex items-center justify-between overflow-hidden rounded-3xl bg-primary px-7 py-10 text-white sm:px-12">
        <div><p className="text-xs font-semibold tracking-[.2em] text-lime-200">PANTAU. PAHAMI. ANTISIPASI.</p>
          <h1 className="my-6 text-4xl font-semibold leading-tight tracking-tight sm:text-6xl">Lebih siap,<br />lebih terlindungi.</h1>
          <p className="max-w-md text-sm leading-7 text-emerald-100">Informasi risiko bencana yang mudah dipahami dalam satu dashboard.</p></div>
        <ShieldCheck className="hidden size-44 text-lime-200/80 sm:block" strokeWidth={0.8} aria-hidden="true" />
      </section>
      <p className="my-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs leading-6 text-amber-900">
      ...
      <section className="grid gap-4 sm:grid-cols-3" aria-label="Ringkasan">
        {[['Wilayah', data ? regions.length : '—', 'Wilayah dalam data'], ['Peringatan', data ? alerts.length : '—', 'Seluruh tingkat risiko'], ['Prediksi AI', 'Belum aktif', 'Menunggu integrasi model']].map(([label, value, caption]) =>
          <article key={label} className="rounded-2xl border border-border bg-white p-6"><p className="text-sm text-muted-foreground">{label}</p><strong className="my-3 block text-3xl font-medium">{value}</strong><span className="text-xs text-muted-foreground">{caption}</span></article>)}
      </section>
      <section className="mt-9">
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="flex flex-wrap items-end gap-3"><label className="text-xs">Wilayah
            <select className="mt-1 block h-10 rounded-lg border border-border bg-white px-3 text-sm" value={selected} onChange={e => setRegion(e.target.value)}><option>Semua wilayah</option>{regions.map(r => <option key={r}>{r}</option>)}</select>
        ...
        {data && filtered.length > 0 && <div className="mb-5 grid gap-4 lg:grid-cols-5">
          <section className="min-w-0 rounded-2xl border border-border bg-white p-4 lg:col-span-3"><h3 className="mb-3 font-semibold">Sebaran wilayah</h3><Suspense fallback={<p className="h-80">Memuat peta…</p>}><RiskMap alerts={filtered} /></Suspense><p className="mt-3 text-xs text-muted-foreground">Peta dasar memerlukan internet. Titik demo menunjukkan lokasi perkiraan kota.</p></section>
          <section className="min-w-0 rounded-2xl border border-border bg-white p-4 lg:col-span-2"><h3 className="mb-3 font-semibold">Jumlah peringatan per tingkat</h3><Suspense fallback={<p>Memuat grafik…</p>}><RiskChart alerts={filtered} /></Suspense></section>
        </div>}
        <div className="grid gap-3">{filtered.map(alert => <article key={alert.id} className="flex gap-4 rounded-xl border border-border bg-white p-5">
          <TriangleAlert className="mt-1 size-6 shrink-0 text-amber-700" aria-hidden="true" /><div className="min-w-0 flex-1">
            <span className="text-xs font-semibold uppercase tracking-wider text-amber-800">{alert.level}</span><h3 className="mt-1 font-semibold">{alert.hazard}</h3><p className="mt-1 text-sm text-muted-foreground">{alert.region}</p>
